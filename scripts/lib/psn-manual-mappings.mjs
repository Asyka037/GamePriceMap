 ...
 *
 * PlayStation's public search page is technically keyless, but its current
 * website terms prohibit automated scraping. Therefore discovery is a human
 * action: a reviewer pastes an exact official product URL, and this pure
 * module validates the bounded input/evidence. Fetching even that exact page
 * remains automation and is separately disabled until the user explicitly
 * accepts the PlayStation-specific terms risk.
 */
...
export function buildPsnMappingCandidate(entry, parsed, {
  ...
  const evidence = sealMappingEvidence({
...
export function createPsnSuggestionDocument({ generatedAt = new Date(), candidates, pending, failures }) {
  ...
  for (const candidate of candidates ?? []) validatePsnMappingCandidate(candidate, { now: generated.valueOf() });
...
export function validatePsnSuggestionDocument(document, { now = Date.now() } = {}) {
  ...
  if (document.schemaVersion !== 1 || document.kind !== PSN_MAPPING_SUGGESTIONS_KIND
    || document.discoveryMode !== 'manual-official-url' || document.stabilityStatus !== 'day-1-only') {
    throw new Error('unsupported PSN mapping suggestions document');
  }
  for (const candidate of document.candidates ?? []) validatePsnMappingCandidate(candidate, { now });
  return document;
}
