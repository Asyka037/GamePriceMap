...
### 2026-07-12 详情头图、地区比价与页脚修正（CodeX）
- [x] 桌面端详情封面与右侧内容等高，移动端恢复横幅比例
- [x] 将 ATL、好评率与 Metacritic 整组移到价格卡片外部上方
- [x] 多渠道地区比价改为单面板渠道切换，避免纵向重复两套地图与表格
- [x] 地图浮窗在离开地图/页面、窗口失焦或按 Esc 后可靠隐藏
- [x] 页脚简介改写为网站定位与核心服务概括
- [x] 更新 lessons，运行单测、validate、构建与产物断言
- [ ] 浏览器验收桌面、390px、地区渠道切换与 hover 隐藏
- 评审：70/70 单测、validate、139 页构建与全量产物断言通过；42 个地区页中 21 个双渠道页均输出 2 个 tab/2 个 panel 且初始只显示 1 个，21 个单渠道页不输出冗余切换器。当前会话的浏览器控制连接未暴露，按 Browser skill 约束未改用其它浏览器驱动，视觉/真实 hover 验收仍待完成；未提交、未推送生产。

### 2026-07-12 导航加载、历史新鲜度、日历图片与外链修正（CodeX）
- [x] `/new-releases/` 直接渲染当前月份，彻底删除可见静态重定向页
- [x] 历史页显示每渠道最后成功检查日，并明确日志只记录变价、Steam/eShop 日更、Xbox 周更
- [x] 国家来源标识去除背景、边框和内边距，只保留国旗与简码
- [x] 日历抓取保存 Steam/Nintendo 图片，合并层保留图片并补测试
- [x] 首页 Coming this month 与月历页渲染图片和可靠降级占位
- [x] 日历改为每日刷新并完成一次真实抓取，使生产首发即有图片
- [x] 全站第三方链接审计：全部使用 `_blank` + `noopener`，增加产物闸门
- [x] 合并上一批未发布修改，更新 lessons，运行 test/validate/build/产物断言
- [ ] 浏览器验收桌面、390px、地区切换/hover、历史说明、日历图片与外链行为
- [ ] 提交并推送 `main`，确认生产部署生效
- 评审：已 rebase 到机器人日更 `303a3da`，确认 2026-07-12 09:28 UTC 的 Steam/eShop 日更成功（42/42、21/21 全量且零失败）。70/70 单测、合并数据 validate、137 页构建通过；真实日历抓取 118 条且图片 118/118。产物断言：`/new-releases/` 零 redirect shell、首页 4/4 发售卡有图、双渠道最后检查日为 2026-07-12、42 个地区页渠道面板规则正确、341 个绝对外链全部 `_blank` + `noopener`。当前会话 Browser 控制工具仍未暴露，待以生产 DOM/HTTP 验收替代后记录残余视觉风险。
...
- [x] 提交并推送 `main`，确认 Cloudflare Pages 生产部署与线上 SEO 生效
- 评审：功能提交 `cca53fe` 已 rebase 到 2026-07-13 日更/周更数据后推送；Cloudflare Production 部署 `48bd4966` success。72/72 单测、validate、137 页构建通过；产物审计覆盖 42 个详情总览 + 42 个区域页 + 42 个历史页。线上抽查首页、Cyberpunk 2077 三类详情页、Stardew Valley eShop 动态国家页及四个列表页全部返回指定 SEO；Cyberpunk 2077（Steam-only）动态输出 Georgia/Pakistan/India，Stardew Valley 按首屏 eShop 数据动态输出 Brazil/Mexico/Japan。
